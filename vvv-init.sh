...
if [ ! -d "frontend/wp-admin" ]; then
	echo 'Installing WordPress (release version) in wp-kristoffe-fr/frontend...'
	if [ ! -d "./frontend" ]; then
		mkdir ./frontend
	fi
	cd ./frontend
	wp core download --locale=en_US --allow-root
	wp core config --dbname="wp-kristoffe-fr" --dbuser=wp --dbpass=wp --dbhost="localhost" --dbprefix=wp_ --locale=en_US --allow-root --extra-php <<PHP
...
PHP
	wp core install --url=wp-kristoffe-fr.test --title="wp-kristoffe-fr" --admin_user=admin --admin_password=password --admin_email=admin@localhost.dev --allow-root
