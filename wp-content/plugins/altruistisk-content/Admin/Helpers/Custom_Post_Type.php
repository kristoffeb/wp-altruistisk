...
namespace Altruistisk\Content\Admin\Helpers;
...
class Custom_Post_Type {
	...
	function __construct( $post_type_names, $options = [] ) {
		...
		// Register taxonomies.
		$this->add_action( 'init', [ &$this, 'register_taxonomies' ], 0 );

		// Register the post type.
		$this->add_action( 'init', [ &$this, 'register_post_type' ], 0 );

		// Register exisiting taxonomies.
		$this->add_action( 'init', [ &$this, 'register_exisiting_taxonomies' ], 0 );
	...
	function add_admin_columns( $columns ) {
		...
		if ( ! isset( $this->columns ) ) {
			...
			if ( is_array( $this->taxonomies ) && in_array( 'post_tag',
					$this->taxonomies ) || $this->post_type_name === 'post'
			) {
				$after = 'tags';
